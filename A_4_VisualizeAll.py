...
model = load_model('cats_and_dogs_small_2.h5')  # cats_and_dogs_small_VGG16.h5
...
# 可视化所有中间滤波器
def show_all_filter(layer_names, activations):
    images_per_row = 16
    size = 64
    # Now let's display our feature maps
    for layer_name, layer_activation in zip(layer_names, activations):
        ...
        for col in range(n_cols):
            for row in range(images_per_row):
                filter_img = generate_pattern(layer_name, col * images_per_row + row)
                display_grid[col * size: (col + 1) * size, row * size: (row + 1) * size, :] = filter_img
        ...
        plt.title(layer_name)
        plt.grid(False)
        plt.imshow(display_grid, aspect='auto')  # viridis
    plt.show()
...
def main():
    ...
    show_all_filter(layer_names, activations)
